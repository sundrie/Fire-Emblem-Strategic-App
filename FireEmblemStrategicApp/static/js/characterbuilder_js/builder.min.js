...
$(chosenOnes.children('td')).each(function(){encoderforAJAX.push($(this).text())});$.ajax({url:'https://alexandreblin.ovh/FireEmblemStrategicApp/php/savebuild.php',method:'POST',data:{build:encoderforAJAX},success:function(data){window.location.replace("https://alexandreblin.ovh/FireEmblemStrategicApp/php/userdownloadfile.php")}})})}
function fillParentList(nameChild){var Parents;$.get('https://alexandreblin.ovh/FireEmblemStrategicApp/HeroesData/Tools/parentsList.txt',function(data){var lignes=data.split("+")
...
$(".formulaireRecherche").on('change','#myParent',function(){$('.chooseAParent').hide();var parentName=$(this).val();var childName=$('.NomHerosBuilder').text();$.get('https://alexandreblin.ovh/FireEmblemStrategicApp/HeroesData/Childrens/'+childName+'.txt',function(data){data=data.split("/");var childClass=data[1];getParentData(childName,parentName,childClass)},'text')});function getParentData(childName,parentName,childClass){$.get('https://alexandreblin.ovh/FireEmblemStrategicApp/HeroesData/'+parentName+'.txt',function(data){data=data.split("/");var parentClass=data[1];completeChildTalent(childName,parentName,childClass,parentClass)},'text')}
